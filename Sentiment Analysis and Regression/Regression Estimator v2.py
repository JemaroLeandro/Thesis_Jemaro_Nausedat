...
apple_model_sinh_sentiment_return_daily = smf.ols(
    formula="np.sinh(apple_daily_abnormal_return) ~ np.sinh(daily_roberta_sum)", data=apple_data_daily).fit()

apple_model_sinh_positivity_return_daily = smf.ols(
    formula="np.sinh(apple_daily_abnormal_return) ~ np.sinh(daily_roberta_pos)", data=apple_data_daily).fit()

apple_model_sinh_negativity_return_daily = smf.ols(
    formula="np.sinh(apple_daily_abnormal_return) ~ np.sinh(daily_roberta_neg)", data=apple_data_daily).fit()

apple_model_sinh_sensitivity_return_daily = smf.ols(
    formula="np.sinh(apple_daily_abnormal_return) ~ np.sinh(daily_sensitivity)", data=apple_data_daily).fit()

apple_model_sinh_sentiment_lagged_return_daily = smf.ols(
    formula="np.sinh(apple_daily_abnormal_return) ~ np.sinh(daily_roberta_sum) + np.sinh(daily_roberta_sum_lag_1)", data=apple_data_daily[1:]).fit()

apple_model_sinh_positivity_lagged_return_daily = smf.ols(
    formula="np.sinh(apple_daily_abnormal_return) ~ np.sinh(daily_roberta_pos)+ np.sinh(daily_roberta_pos_lag_1)", data=apple_data_daily[1:]).fit()

apple_model_sinh_negativity_lagged_return_daily = smf.ols(
    formula="np.sinh(apple_daily_abnormal_return) ~ np.sinh(daily_roberta_neg)+ np.sinh(daily_roberta_neg_lag_1)", data=apple_data_daily[1:]).fit()

apple_model_sinh_sensitivity_lagged_return_daily = smf.ols(
    formula="np.sinh(apple_daily_abnormal_return) ~ np.sinh(daily_sensitivity)+ np.sinh(daily_sensitivity_lag_1)", data=apple_data_daily[1:]).fit()


apple_model_sinh_sentiment_norm_daily = smf.ols(
    formula="np.sinh(apple_daily_return) ~ np.sinh(daily_roberta_sum) + np.sinh(sp500_daily_returns)", data=apple_data_daily).fit()

apple_model_sinh_positivity_norm_daily = smf.ols(
    formula="np.sinh(apple_daily_return) ~ np.sinh(daily_roberta_pos) + np.sinh(sp500_daily_returns)", data=apple_data_daily).fit()

apple_model_sinh_negativity_norm_daily = smf.ols(
    formula="np.sinh(apple_daily_return) ~ np.sinh(daily_roberta_neg) + np.sinh(sp500_daily_returns)", data=apple_data_daily).fit()

apple_model_sinh_sensitivity_norm_daily = smf.ols(
    formula="np.sinh(apple_daily_return) ~ np.sinh(daily_sensitivity) + np.sinh(sp500_daily_returns)", data=apple_data_daily).fit()

apple_model_sinh_sentiment_lagged_norm_daily = smf.ols(
    formula="np.sinh(apple_daily_return) ~ np.sinh(daily_roberta_sum) + np.sinh(daily_roberta_sum_lag_1) + np.sinh(sp500_daily_returns)", data=apple_data_daily[1:]).fit()

apple_model_sinh_positivity_lagged_norm_daily = smf.ols(
    formula="np.sinh(apple_daily_return) ~ np.sinh(daily_roberta_pos)+ np.sinh(daily_roberta_pos_lag_1) + np.sinh(sp500_daily_returns)", data=apple_data_daily[1:]).fit()

apple_model_sinh_negativity_lagged_norm_daily = smf.ols(
    formula="np.sinh(apple_daily_return) ~ np.sinh(daily_roberta_neg)+ np.sinh(daily_roberta_neg_lag_1) + np.sinh(sp500_daily_returns)", data=apple_data_daily[1:]).fit()

apple_model_sinh_sensitivity_lagged_norm_daily = smf.ols(
    formula="np.sinh(apple_daily_return) ~ np.sinh(daily_sensitivity)+ np.sinh(daily_sensitivity_lag_1) + np.sinh(sp500_daily_returns)", data=apple_data_daily[1:]).fit()

# ---log


apple_model_log_positivity_return_daily = smf.ols(
    formula="np.log(apple_daily_abnormal_return) ~ np.log(daily_roberta_pos)", data=apple_data_daily).fit()

apple_model_log_negativity_return_daily = smf.ols(
    formula="np.log(apple_daily_abnormal_return) ~ np.log(daily_roberta_neg)", data=apple_data_daily).fit()

apple_model_log_positivity_lagged_return_daily = smf.ols(
    formula="np.log(apple_daily_abnormal_return) ~ np.log(daily_roberta_pos)+ np.log(daily_roberta_pos_lag_1)", data=apple_data_daily[1:]).fit()

apple_model_log_negativity_lagged_return_daily = smf.ols(
    formula="np.log(apple_daily_abnormal_return) ~ np.log(daily_roberta_neg)+ np.log(daily_roberta_neg_lag_1)", data=apple_data_daily[1:]).fit()


apple_model_log_positivity_norm_daily = smf.ols(
    formula="np.log(apple_daily_return) ~ np.log(daily_roberta_pos) + np.log(sp500_daily_returns)", data=apple_data_daily).fit()

apple_model_log_negativity_norm_daily = smf.ols(
    formula="np.log(apple_daily_return) ~ np.log(daily_roberta_neg) + np.log(sp500_daily_returns)", data=apple_data_daily).fit()

apple_model_log_positivity_lagged_norm_daily = smf.ols(
    formula="np.log(apple_daily_return) ~ np.log(daily_roberta_pos)+ np.log(daily_roberta_pos_lag_1) + np.log(sp500_daily_returns)", data=apple_data_daily[1:]).fit()

apple_model_log_negativity_lagged_norm_daily = smf.ols(
    formula="np.log(apple_daily_return) ~ np.log(daily_roberta_neg)+ np.log(daily_roberta_neg_lag_1) + np.log(sp500_daily_returns)", data=apple_data_daily[1:]).fit()

# ---sqrt


apple_model_sqrt_positivity_return_daily = smf.ols(
    formula="np.sqrt(apple_daily_abnormal_return) ~ np.sqrt(daily_roberta_pos)", data=apple_data_daily).fit()

apple_model_sqrt_negativity_return_daily = smf.ols(
    formula="np.sqrt(apple_daily_abnormal_return) ~ np.sqrt(daily_roberta_neg)", data=apple_data_daily).fit()

apple_model_sqrt_positivity_lagged_return_daily = smf.ols(
    formula="np.sqrt(apple_daily_abnormal_return) ~ np.sqrt(daily_roberta_pos)+ np.sqrt(daily_roberta_pos_lag_1)", data=apple_data_daily[1:]).fit()

apple_model_sqrt_negativity_lagged_return_daily = smf.ols(
    formula="np.sqrt(apple_daily_abnormal_return) ~ np.sqrt(daily_roberta_neg)+ np.sqrt(daily_roberta_neg_lag_1)", data=apple_data_daily[1:]).fit()


apple_model_sqrt_positivity_norm_daily = smf.ols(
    formula="np.sqrt(apple_daily_return) ~ np.sqrt(daily_roberta_pos) + np.sqrt(sp500_daily_returns)", data=apple_data_daily).fit()

apple_model_sqrt_negativity_norm_daily = smf.ols(
    formula="np.sqrt(apple_daily_return) ~ np.sqrt(daily_roberta_neg) + np.sqrt(sp500_daily_returns)", data=apple_data_daily).fit()

apple_model_sqrt_positivity_lagged_norm_daily = smf.ols(
    formula="np.sqrt(apple_daily_return) ~ np.sqrt(daily_roberta_pos)+ np.sqrt(daily_roberta_pos_lag_1) + np.sqrt(sp500_daily_returns)", data=apple_data_daily[1:]).fit()

apple_model_sqrt_negativity_lagged_norm_daily = smf.ols(
    formula="np.sqrt(apple_daily_return) ~ np.sqrt(daily_roberta_neg)+ np.sqrt(daily_roberta_neg_lag_1) + np.sqrt(sp500_daily_returns)", data=apple_data_daily[1:]).fit()
...
apple_model_linear_sentiment_norm_weekly = smf.ols(
    formula="weekly_return ~ roberta_sum + sp500_weekly_returns", data=apple_data_weekly).fit()

apple_model_linear_positivity_norm_weekly = smf.ols(
    formula="weekly_return ~ roberta_pos + sp500_weekly_returns", data=apple_data_weekly).fit()

apple_model_linear_negativity_norm_weekly = smf.ols(
    formula="weekly_return ~ roberta_neg + sp500_weekly_returns", data=apple_data_weekly).fit()

apple_model_linear_sensitivity_norm_weekly = smf.ols(
    formula="weekly_return ~ sensitivity + sp500_weekly_returns", data=apple_data_weekly[1:]).fit()

apple_model_linear_sentiment_lagged_norm_weekly = smf.ols(
    formula="weekly_return ~ roberta_sum + roberta_sum_lag_1 + sp500_weekly_returns", data=apple_data_weekly[1:]).fit()

apple_model_linear_positivity_lagged_norm_weekly = smf.ols(
    formula="weekly_return ~ roberta_pos + roberta_pos_lag_1 + sp500_weekly_returns", data=apple_data_weekly[1:]).fit()

apple_model_linear_negativity_lagged_norm_weekly = smf.ols(
    formula="weekly_return ~ roberta_neg + roberta_neg_lag_1 + sp500_weekly_returns", data=apple_data_weekly[1:]).fit()

apple_model_linear_sensitivity_lagged_norm_weekly = smf.ols(
    formula="weekly_return ~ sensitivity + sensitivity_lag_1 + sp500_weekly_returns", data=apple_data_weekly[2:]).fit()

# ---sinh

apple_model_sinh_sentiment_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~ np.sinh(roberta_sum)", data=apple_data_weekly).fit()

apple_model_sinh_positivity_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~ np.sinh(roberta_pos)", data=apple_data_weekly).fit()

apple_model_sinh_negativity_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~ np.sinh(roberta_neg)", data=apple_data_weekly).fit()

apple_model_sinh_sensitivity_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~  np.sinh(sensitivity)", data=apple_data_weekly[1:]).fit()

apple_model_sinh_sentiment_lagged_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~  np.sinh(roberta_sum) +  np.sinh(roberta_sum_lag_1)", data=apple_data_weekly[1:]).fit()

apple_model_sinh_positivity_lagged_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~ np.sinh(roberta_pos) +  np.sinh(roberta_pos_lag_1)", data=apple_data_weekly[1:]).fit()

apple_model_sinh_negativity_lagged_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~ np.sinh(roberta_neg) +  np.sinh(roberta_neg_lag_1)", data=apple_data_weekly[1:]).fit()

apple_model_sinh_sensitivity_lagged_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~  np.sinh(sensitivity) +  np.sinh(sensitivity_lag_1)", data=apple_data_weekly[2:]).fit()


apple_model_sinh_sentiment_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~ np.sinh(roberta_sum)", data=apple_data_weekly).fit()

apple_model_sinh_positivity_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~ np.sinh(roberta_pos)", data=apple_data_weekly).fit()

apple_model_sinh_negativity_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~ np.sinh(roberta_neg)", data=apple_data_weekly).fit()

apple_model_sinh_sensitivity_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~  np.sinh(sensitivity)", data=apple_data_weekly[1:]).fit()

apple_model_sinh_sentiment_lagged_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~  np.sinh(roberta_sum) +  np.sinh(roberta_sum_lag_1)", data=apple_data_weekly[1:]).fit()

apple_model_sinh_positivity_lagged_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~ np.sinh(roberta_pos) +  np.sinh(roberta_pos_lag_1)", data=apple_data_weekly[1:]).fit()

apple_model_sinh_negativity_lagged_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~ np.sinh(roberta_neg) +  np.sinh(roberta_neg_lag_1)", data=apple_data_weekly[1:]).fit()

apple_model_sinh_sensitivity_lagged_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~  np.sinh(sensitivity) +  np.sinh(sensitivity_lag_1)", data=apple_data_weekly[2:]).fit()


apple_model_sinh_sentiment_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~ np.sinh(roberta_sum) + np.sinh(sp500_weekly_returns)", data=apple_data_weekly).fit()

apple_model_sinh_positivity_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~ np.sinh(roberta_pos) + np.sinh(sp500_weekly_returns)", data=apple_data_weekly).fit()

apple_model_sinh_negativity_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~ np.sinh(roberta_neg) + np.sinh(sp500_weekly_returns)", data=apple_data_weekly).fit()

apple_model_sinh_sensitivity_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~  np.sinh(sensitivity) + np.sinh(sp500_weekly_returns)", data=apple_data_weekly[1:]).fit()

apple_model_sinh_sentiment_lagged_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~  np.sinh(roberta_sum) +  np.sinh(roberta_sum_lag_1) + np.sinh(sp500_weekly_returns)", data=apple_data_weekly[1:]).fit()

apple_model_sinh_positivity_lagged_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~ np.sinh(roberta_pos) +  np.sinh(roberta_pos_lag_1) + np.sinh(sp500_weekly_returns)", data=apple_data_weekly[1:]).fit()

apple_model_sinh_negativity_lagged_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~ np.sinh(roberta_neg) +  np.sinh(roberta_neg_lag_1) + np.sinh(sp500_weekly_returns)", data=apple_data_weekly[1:]).fit()

apple_model_sinh_sensitivity_lagged_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~  np.sinh(sensitivity) +  np.sinh(sensitivity_lag_1) + np.sinh(sp500_weekly_returns)", data=apple_data_weekly[2:]).fit()
...
apple_model_log_positivity_return_weekly = smf.ols(
    formula="np.log(weekly_abnormal_return) ~ np.log(roberta_pos)", data=apple_data_weekly).fit()

apple_model_log_negativity_return_weekly = smf.ols(
    formula="np.log(weekly_abnormal_return) ~ np.log(roberta_neg)", data=apple_data_weekly).fit()

apple_model_log_positivity_lagged_return_weekly = smf.ols(
    formula="np.log(weekly_abnormal_return) ~ np.log(roberta_pos) +  np.log(roberta_pos_lag_1)", data=apple_data_weekly[1:]).fit()

apple_model_log_negativity_lagged_return_weekly = smf.ols(
    formula="np.log(weekly_abnormal_return) ~ np.log(roberta_neg) +  np.log(roberta_neg_lag_1)", data=apple_data_weekly[1:]).fit()


apple_model_log_positivity_volatility_weekly = smf.ols(
    formula="np.log(average_volatility) ~ np.log(roberta_pos)", data=apple_data_weekly).fit()

apple_model_log_negativity_volatility_weekly = smf.ols(
    formula="np.log(average_volatility) ~ np.log(roberta_neg)", data=apple_data_weekly).fit()

apple_model_log_positivity_lagged_volatility_weekly = smf.ols(
    formula="np.log(average_volatility) ~ np.log(roberta_pos) +  np.log(roberta_pos_lag_1)", data=apple_data_weekly[1:]).fit()

apple_model_log_negativity_lagged_volatility_weekly = smf.ols(
    formula="np.log(average_volatility) ~ np.log(roberta_neg) +  np.log(roberta_neg_lag_1)", data=apple_data_weekly[1:]).fit()


apple_model_log_positivity_norm_weekly = smf.ols(
    formula="np.log(weekly_return) ~ np.log(roberta_pos) + np.log(sp500_weekly_returns)", data=apple_data_weekly).fit()

apple_model_log_negativity_norm_weekly = smf.ols(
    formula="np.log(weekly_return) ~ np.log(roberta_neg) + np.log(sp500_weekly_returns)", data=apple_data_weekly).fit()

apple_model_log_positivity_lagged_norm_weekly = smf.ols(
    formula="np.log(weekly_return) ~ np.log(roberta_pos) +  np.log(roberta_pos_lag_1) + np.log(sp500_weekly_returns)", data=apple_data_weekly[1:]).fit()

apple_model_log_negativity_lagged_norm_weekly = smf.ols(
    formula="np.log(weekly_return) ~ np.log(roberta_neg) +  np.log(roberta_neg_lag_1) + np.log(sp500_weekly_returns)", data=apple_data_weekly[1:]).fit()

# ---sqrt

apple_model_sqrt_positivity_return_weekly = smf.ols(
    formula="np.sqrt(weekly_abnormal_return) ~ np.sqrt(roberta_pos)", data=apple_data_weekly).fit()

apple_model_sqrt_negativity_return_weekly = smf.ols(
    formula="np.sqrt(weekly_abnormal_return) ~ np.sqrt(roberta_neg)", data=apple_data_weekly).fit()

apple_model_sqrt_positivity_lagged_return_weekly = smf.ols(
    formula="np.sqrt(weekly_abnormal_return) ~ np.sqrt(roberta_pos) +  np.sqrt(roberta_pos_lag_1)", data=apple_data_weekly[1:]).fit()

apple_model_sqrt_negativity_lagged_return_weekly = smf.ols(
    formula="np.sqrt(weekly_abnormal_return) ~ np.sqrt(roberta_neg) +  np.sqrt(roberta_neg_lag_1)", data=apple_data_weekly[1:]).fit()


apple_model_sqrt_positivity_volatility_weekly = smf.ols(
    formula="np.sqrt(average_volatility) ~ np.sqrt(roberta_pos)", data=apple_data_weekly).fit()

apple_model_sqrt_negativity_volatility_weekly = smf.ols(
    formula="np.sqrt(average_volatility) ~ np.sqrt(roberta_neg)", data=apple_data_weekly).fit()

apple_model_sqrt_positivity_lagged_volatility_weekly = smf.ols(
    formula="np.sqrt(average_volatility) ~ np.sqrt(roberta_pos) +  np.sqrt(roberta_pos_lag_1)", data=apple_data_weekly[1:]).fit()

apple_model_sqrt_negativity_lagged_volatility_weekly = smf.ols(
    formula="np.sqrt(average_volatility) ~ np.sqrt(roberta_neg) +  np.sqrt(roberta_neg_lag_1)", data=apple_data_weekly[1:]).fit()


apple_model_sqrt_positivity_norm_weekly = smf.ols(
    formula="np.sqrt(weekly_return) ~ np.sqrt(roberta_pos) + np.sqrt(sp500_weekly_returns)", data=apple_data_weekly).fit()

apple_model_sqrt_negativity_norm_weekly = smf.ols(
    formula="np.sqrt(weekly_return) ~ np.sqrt(roberta_neg) + np.sqrt(sp500_weekly_returns)", data=apple_data_weekly).fit()

apple_model_sqrt_positivity_lagged_norm_weekly = smf.ols(
    formula="np.sqrt(weekly_return) ~ np.sqrt(roberta_pos) +  np.sqrt(roberta_pos_lag_1) + np.sqrt(sp500_weekly_returns)", data=apple_data_weekly[1:]).fit()

apple_model_sqrt_negativity_lagged_norm_weekly = smf.ols(
    formula="np.sqrt(weekly_return) ~ np.sqrt(roberta_neg) +  np.sqrt(roberta_neg_lag_1) + np.sqrt(sp500_weekly_returns)", data=apple_data_weekly[1:]).fit()
...
blizzard_ent_model_sinh_sentiment_return_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_abnormal_return) ~ np.sinh(daily_roberta_sum)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_sinh_positivity_return_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_abnormal_return) ~ np.sinh(daily_roberta_pos)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_sinh_negativity_return_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_abnormal_return) ~ np.sinh(daily_roberta_neg)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_sinh_sensitivity_return_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_abnormal_return) ~ np.sinh(daily_sensitivity)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_sinh_sentiment_lagged_return_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_abnormal_return) ~ np.sinh(daily_roberta_sum) + np.sinh(daily_roberta_sum_lag_1)", data=blizzard_ent_data_daily[1:]).fit()

blizzard_ent_model_sinh_positivity_lagged_return_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_abnormal_return) ~ np.sinh(daily_roberta_pos)+ np.sinh(daily_roberta_pos_lag_1)", data=blizzard_ent_data_daily[1:]).fit()

blizzard_ent_model_sinh_negativity_lagged_return_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_abnormal_return) ~ np.sinh(daily_roberta_neg)+ np.sinh(daily_roberta_neg_lag_1)", data=blizzard_ent_data_daily[1:]).fit()

blizzard_ent_model_sinh_sensitivity_lagged_return_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_abnormal_return) ~ np.sinh(daily_sensitivity)+ np.sinh(daily_sensitivity_lag_1)", data=blizzard_ent_data_daily[1:]).fit()


blizzard_ent_model_sinh_sentiment_norm_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_return) ~ np.sinh(daily_roberta_sum) + np.sinh(sp500_daily_returns)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_sinh_positivity_norm_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_return) ~ np.sinh(daily_roberta_pos) + np.sinh(sp500_daily_returns)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_sinh_negativity_norm_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_return) ~ np.sinh(daily_roberta_neg) + np.sinh(sp500_daily_returns)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_sinh_sensitivity_norm_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_return) ~ np.sinh(daily_sensitivity) + np.sinh(sp500_daily_returns)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_sinh_sentiment_lagged_norm_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_return) ~ np.sinh(daily_roberta_sum) + np.sinh(daily_roberta_sum_lag_1) + np.sinh(sp500_daily_returns)", data=blizzard_ent_data_daily[1:]).fit()

blizzard_ent_model_sinh_positivity_lagged_norm_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_return) ~ np.sinh(daily_roberta_pos)+ np.sinh(daily_roberta_pos_lag_1) + np.sinh(sp500_daily_returns)", data=blizzard_ent_data_daily[1:]).fit()

blizzard_ent_model_sinh_negativity_lagged_norm_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_return) ~ np.sinh(daily_roberta_neg)+ np.sinh(daily_roberta_neg_lag_1) + np.sinh(sp500_daily_returns)", data=blizzard_ent_data_daily[1:]).fit()

blizzard_ent_model_sinh_sensitivity_lagged_norm_daily = smf.ols(
    formula="np.sinh(blizzard_ent_daily_return) ~ np.sinh(daily_sensitivity)+ np.sinh(daily_sensitivity_lag_1) + np.sinh(sp500_daily_returns)", data=blizzard_ent_data_daily[1:]).fit()

# ---log


blizzard_ent_model_log_positivity_return_daily = smf.ols(
    formula="np.log(blizzard_ent_daily_abnormal_return) ~ np.log(daily_roberta_pos)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_log_negativity_return_daily = smf.ols(
    formula="np.log(blizzard_ent_daily_abnormal_return) ~ np.log(daily_roberta_neg)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_log_positivity_lagged_return_daily = smf.ols(
    formula="np.log(blizzard_ent_daily_abnormal_return) ~ np.log(daily_roberta_pos)+ np.log(daily_roberta_pos_lag_1)", data=blizzard_ent_data_daily[1:]).fit()

blizzard_ent_model_log_negativity_lagged_return_daily = smf.ols(
    formula="np.log(blizzard_ent_daily_abnormal_return) ~ np.log(daily_roberta_neg)+ np.log(daily_roberta_neg_lag_1)", data=blizzard_ent_data_daily[1:]).fit()


blizzard_ent_model_log_positivity_norm_daily = smf.ols(
    formula="np.log(blizzard_ent_daily_return) ~ np.log(daily_roberta_pos) + np.log(sp500_daily_returns)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_log_negativity_norm_daily = smf.ols(
    formula="np.log(blizzard_ent_daily_return) ~ np.log(daily_roberta_neg) + np.log(sp500_daily_returns)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_log_positivity_lagged_norm_daily = smf.ols(
    formula="np.log(blizzard_ent_daily_return) ~ np.log(daily_roberta_pos)+ np.log(daily_roberta_pos_lag_1) + np.log(sp500_daily_returns)", data=blizzard_ent_data_daily[1:]).fit()

blizzard_ent_model_log_negativity_lagged_norm_daily = smf.ols(
    formula="np.log(blizzard_ent_daily_return) ~ np.log(daily_roberta_neg)+ np.log(daily_roberta_neg_lag_1) + np.log(sp500_daily_returns)", data=blizzard_ent_data_daily[1:]).fit()

# ---sqrt


blizzard_ent_model_sqrt_positivity_return_daily = smf.ols(
    formula="np.sqrt(blizzard_ent_daily_abnormal_return) ~ np.sqrt(daily_roberta_pos)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_sqrt_negativity_return_daily = smf.ols(
    formula="np.sqrt(blizzard_ent_daily_abnormal_return) ~ np.sqrt(daily_roberta_neg)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_sqrt_positivity_lagged_return_daily = smf.ols(
    formula="np.sqrt(blizzard_ent_daily_abnormal_return) ~ np.sqrt(daily_roberta_pos)+ np.sqrt(daily_roberta_pos_lag_1)", data=blizzard_ent_data_daily[1:]).fit()

blizzard_ent_model_sqrt_negativity_lagged_return_daily = smf.ols(
    formula="np.sqrt(blizzard_ent_daily_abnormal_return) ~ np.sqrt(daily_roberta_neg)+ np.sqrt(daily_roberta_neg_lag_1)", data=blizzard_ent_data_daily[1:]).fit()


blizzard_ent_model_sqrt_positivity_norm_daily = smf.ols(
    formula="np.sqrt(blizzard_ent_daily_return) ~ np.sqrt(daily_roberta_pos) + np.sqrt(sp500_daily_returns)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_sqrt_negativity_norm_daily = smf.ols(
    formula="np.sqrt(blizzard_ent_daily_return) ~ np.sqrt(daily_roberta_neg) + np.sqrt(sp500_daily_returns)", data=blizzard_ent_data_daily).fit()

blizzard_ent_model_sqrt_positivity_lagged_norm_daily = smf.ols(
    formula="np.sqrt(blizzard_ent_daily_return) ~ np.sqrt(daily_roberta_pos)+ np.sqrt(daily_roberta_pos_lag_1) + np.sqrt(sp500_daily_returns)", data=blizzard_ent_data_daily[1:]).fit()

blizzard_ent_model_sqrt_negativity_lagged_norm_daily = smf.ols(
    formula="np.sqrt(blizzard_ent_daily_return) ~ np.sqrt(daily_roberta_neg)+ np.sqrt(daily_roberta_neg_lag_1) + np.sqrt(sp500_daily_returns)", data=blizzard_ent_data_daily[1:]).fit()
...
blizzard_ent_model_linear_sentiment_norm_weekly = smf.ols(
    formula="weekly_return ~ roberta_sum + sp500_weekly_returns", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_linear_positivity_norm_weekly = smf.ols(
    formula="weekly_return ~ roberta_pos + sp500_weekly_returns", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_linear_negativity_norm_weekly = smf.ols(
    formula="weekly_return ~ roberta_neg + sp500_weekly_returns", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_linear_sensitivity_norm_weekly = smf.ols(
    formula="weekly_return ~ sensitivity + sp500_weekly_returns", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_linear_sentiment_lagged_norm_weekly = smf.ols(
    formula="weekly_return ~ roberta_sum + roberta_sum_lag_1 + sp500_weekly_returns", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_linear_positivity_lagged_norm_weekly = smf.ols(
    formula="weekly_return ~ roberta_pos + roberta_pos_lag_1 + sp500_weekly_returns", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_linear_negativity_lagged_norm_weekly = smf.ols(
    formula="weekly_return ~ roberta_neg + roberta_neg_lag_1 + sp500_weekly_returns", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_linear_sensitivity_lagged_norm_weekly = smf.ols(
    formula="weekly_return ~ sensitivity + sensitivity_lag_1 + sp500_weekly_returns", data=blizzard_ent_data_weekly[2:]).fit()

# ---sinh

blizzard_ent_model_sinh_sentiment_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~ np.sinh(roberta_sum)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sinh_positivity_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~ np.sinh(roberta_pos)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sinh_negativity_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~ np.sinh(roberta_neg)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sinh_sensitivity_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~  np.sinh(sensitivity)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sinh_sentiment_lagged_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~  np.sinh(roberta_sum) +  np.sinh(roberta_sum_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sinh_positivity_lagged_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~ np.sinh(roberta_pos) +  np.sinh(roberta_pos_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sinh_negativity_lagged_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~ np.sinh(roberta_neg) +  np.sinh(roberta_neg_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sinh_sensitivity_lagged_return_weekly = smf.ols(
    formula="np.sinh(weekly_abnormal_return) ~  np.sinh(sensitivity) +  np.sinh(sensitivity_lag_1)", data=blizzard_ent_data_weekly[2:]).fit()


blizzard_ent_model_sinh_sentiment_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~ np.sinh(roberta_sum)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sinh_positivity_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~ np.sinh(roberta_pos)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sinh_negativity_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~ np.sinh(roberta_neg)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sinh_sensitivity_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~  np.sinh(sensitivity)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sinh_sentiment_lagged_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~  np.sinh(roberta_sum) +  np.sinh(roberta_sum_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sinh_positivity_lagged_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~ np.sinh(roberta_pos) +  np.sinh(roberta_pos_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sinh_negativity_lagged_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~ np.sinh(roberta_neg) +  np.sinh(roberta_neg_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sinh_sensitivity_lagged_volatility_weekly = smf.ols(
    formula="np.sinh(average_volatility) ~  np.sinh(sensitivity) +  np.sinh(sensitivity_lag_1)", data=blizzard_ent_data_weekly[2:]).fit()


blizzard_ent_model_sinh_sentiment_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~ np.sinh(roberta_sum) + np.sinh(sp500_weekly_returns)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sinh_positivity_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~ np.sinh(roberta_pos) + np.sinh(sp500_weekly_returns)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sinh_negativity_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~ np.sinh(roberta_neg) + np.sinh(sp500_weekly_returns)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sinh_sensitivity_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~  np.sinh(sensitivity) + np.sinh(sp500_weekly_returns)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sinh_sentiment_lagged_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~  np.sinh(roberta_sum) +  np.sinh(roberta_sum_lag_1) + np.sinh(sp500_weekly_returns)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sinh_positivity_lagged_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~ np.sinh(roberta_pos) +  np.sinh(roberta_pos_lag_1) + np.sinh(sp500_weekly_returns)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sinh_negativity_lagged_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~ np.sinh(roberta_neg) +  np.sinh(roberta_neg_lag_1) + np.sinh(sp500_weekly_returns)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sinh_sensitivity_lagged_norm_weekly = smf.ols(
    formula="np.sinh(weekly_return) ~  np.sinh(sensitivity) +  np.sinh(sensitivity_lag_1) + np.sinh(sp500_weekly_returns)", data=blizzard_ent_data_weekly[2:]).fit()
...
blizzard_ent_model_log_positivity_return_weekly = smf.ols(
    formula="np.log(weekly_abnormal_return) ~ np.log(roberta_pos)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_log_negativity_return_weekly = smf.ols(
    formula="np.log(weekly_abnormal_return) ~ np.log(roberta_neg)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_log_positivity_lagged_return_weekly = smf.ols(
    formula="np.log(weekly_abnormal_return) ~ np.log(roberta_pos) +  np.log(roberta_pos_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_log_negativity_lagged_return_weekly = smf.ols(
    formula="np.log(weekly_abnormal_return) ~ np.log(roberta_neg) +  np.log(roberta_neg_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()


blizzard_ent_model_log_positivity_volatility_weekly = smf.ols(
    formula="np.log(average_volatility) ~ np.log(roberta_pos)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_log_negativity_volatility_weekly = smf.ols(
    formula="np.log(average_volatility) ~ np.log(roberta_neg)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_log_positivity_lagged_volatility_weekly = smf.ols(
    formula="np.log(average_volatility) ~ np.log(roberta_pos) +  np.log(roberta_pos_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_log_negativity_lagged_volatility_weekly = smf.ols(
    formula="np.log(average_volatility) ~ np.log(roberta_neg) +  np.log(roberta_neg_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()


blizzard_ent_model_log_positivity_norm_weekly = smf.ols(
    formula="np.log(weekly_return) ~ np.log(roberta_pos) + np.log(sp500_weekly_returns)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_log_negativity_norm_weekly = smf.ols(
    formula="np.log(weekly_return) ~ np.log(roberta_neg) + np.log(sp500_weekly_returns)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_log_positivity_lagged_norm_weekly = smf.ols(
    formula="np.log(weekly_return) ~ np.log(roberta_pos) +  np.log(roberta_pos_lag_1) + np.log(sp500_weekly_returns)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_log_negativity_lagged_norm_weekly = smf.ols(
    formula="np.log(weekly_return) ~ np.log(roberta_neg) +  np.log(roberta_neg_lag_1) + np.log(sp500_weekly_returns)", data=blizzard_ent_data_weekly[1:]).fit()

# ---sqrt

blizzard_ent_model_sqrt_positivity_return_weekly = smf.ols(
    formula="np.sqrt(weekly_abnormal_return) ~ np.sqrt(roberta_pos)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sqrt_negativity_return_weekly = smf.ols(
    formula="np.sqrt(weekly_abnormal_return) ~ np.sqrt(roberta_neg)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sqrt_positivity_lagged_return_weekly = smf.ols(
    formula="np.sqrt(weekly_abnormal_return) ~ np.sqrt(roberta_pos) +  np.sqrt(roberta_pos_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sqrt_negativity_lagged_return_weekly = smf.ols(
    formula="np.sqrt(weekly_abnormal_return) ~ np.sqrt(roberta_neg) +  np.sqrt(roberta_neg_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()


blizzard_ent_model_sqrt_positivity_volatility_weekly = smf.ols(
    formula="np.sqrt(average_volatility) ~ np.sqrt(roberta_pos)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sqrt_negativity_volatility_weekly = smf.ols(
    formula="np.sqrt(average_volatility) ~ np.sqrt(roberta_neg)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sqrt_positivity_lagged_volatility_weekly = smf.ols(
    formula="np.sqrt(average_volatility) ~ np.sqrt(roberta_pos) +  np.sqrt(roberta_pos_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sqrt_negativity_lagged_volatility_weekly = smf.ols(
    formula="np.sqrt(average_volatility) ~ np.sqrt(roberta_neg) +  np.sqrt(roberta_neg_lag_1)", data=blizzard_ent_data_weekly[1:]).fit()


blizzard_ent_model_sqrt_positivity_norm_weekly = smf.ols(
    formula="np.sqrt(weekly_return) ~ np.sqrt(roberta_pos) + np.sqrt(sp500_weekly_returns)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sqrt_negativity_norm_weekly = smf.ols(
    formula="np.sqrt(weekly_return) ~ np.sqrt(roberta_neg) + np.sqrt(sp500_weekly_returns)", data=blizzard_ent_data_weekly).fit()

blizzard_ent_model_sqrt_positivity_lagged_norm_weekly = smf.ols(
    formula="np.sqrt(weekly_return) ~ np.sqrt(roberta_pos) +  np.sqrt(roberta_pos_lag_1) + np.sqrt(sp500_weekly_returns)", data=blizzard_ent_data_weekly[1:]).fit()

blizzard_ent_model_sqrt_negativity_lagged_norm_weekly = smf.ols(
    formula="np.sqrt(weekly_return) ~ np.sqrt(roberta_neg) +  np.sqrt(roberta_neg_lag_1) + np.sqrt(sp500_weekly_returns)", data=blizzard_ent_data_weekly[1:]).fit()
